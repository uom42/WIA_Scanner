...
internal class ScannerDevice : WIADeviceBase
{
	...
	{
		get => _props
			.eSearchWiaPropertyByID(WIA_PROP_ID.WIA_DPS_DOCUMENT_HANDLING_SELECT)?
			.eGetWiaPropertyValue((WIA_DPS_DOCUMENT_HANDLING_SELECT)0);
		set
		{
			if (Caps.DocumentHandlingCaps_MultuSources)
				_props
					.eSearchWiaPropertyByID(WIA_PROP_ID.WIA_DPS_DOCUMENT_HANDLING_SELECT)?
					.eSetWiaPropertyValue(value!);
		}
	}
	...
	{
		get => _props
			.eSearchWiaPropertyByID(WIA_PROP_ID.WIA_DPS_DOCUMENT_HANDLING_STATUS)?
			.eGetWiaPropertyValue((WIA_DPS_DOCUMENT_HANDLING_STATUS)0);
	}
	...
	{
		get => _props.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_PAGES, 0);
		set => _props.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_PAGES, value);
	}


	public WIA_PREVIEW_MODES Preview
	{
		get => _props.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_PREVIEW, WIA_PREVIEW_MODES.WIA_PREVIEW_SCAN);
		set => _props.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_PREVIEW, value);
	}
	...
	public TimeSpan MaxScanTime
	{
		get
		{
			...
			int milisec = _props.eGetWiaPropertyValue(WIA_PROP_ID.WIA_DPS_MAX_SCAN_TIME, 0);
	...
	{
		get => ScanCommand.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_CUR_INTENT, (WIA_IPS_CUR_INTENT)0);
		set => ScanCommand.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_CUR_INTENT, value);
	}
	...
	public Point ScanDPI
	{
		get
		{
			Item sc = ScanCommand;
			return new(
			sc.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XRES, 0),
			sc.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YRES, 0)
			);
		}
		set
		{
			Item sc = ScanCommand;
			sc.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XRES, value.X);
			sc.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YRES, value.Y);
		}
	}
	...
	public Rectangle CropRectPx
	{
		get
		{
			...
			Rectangle rcCrop = new(
				cmd.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XPOS, -1),
				cmd.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YPOS, -1),
				cmd.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XEXTENT, -1),
				cmd.eGetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YEXTENT, -1));
		...
		{
			Item cmd = ScanCommand;
			cmd.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XPOS, value.Left);
			cmd.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YPOS, value.Top);
			cmd.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_XEXTENT, value.Width);
			cmd.eSetWiaPropertyValue(WIA_PROP_ID.WIA_IPS_YEXTENT, value.Height);
		}
	}
			...
			)
	{
		...
		try // Начинаем сканировать страницу
		{
			...
			Rectangle defaultPageRectPix = new(
				new Point(0, 0),
				sourceSizeIn.eInches_To_Pixels(dpi));
			...
			/*
			scanCommand.eSetPropertyValue(WIA_PROP_ID.WIA_IPS_BRIGHTNESS, brightnessPercents);
			scanCommand.eSetPropertyValue(WIA_PROP_ID.WIA_IPS_CONTRAST, contrastPercents);
			  ...
			  switch (Scanner.Format)
		{
			...
			do
			{
				...
				else if (onPageScanToFileCompleted != null)
				{
					...
					var wiaFormat = wf.eGetOutputInfoForImageFormat();
		...
		)
	{
		...
		do
		{
			...
			{
				Point dpi = new(
					(int)wiaImage.HorizontalResolution.eRound(0),
					(int)wiaImage.VerticalResolution.eRound(0));

				Size imageSizePx = new(wiaImage.Width, wiaImage.Height);
				SizeF imageSizeIn = imageSizePx.ePixels_To_Inches(dpi);
