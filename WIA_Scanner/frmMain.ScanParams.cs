...
partial class frmMain
{
	...
	private static readonly ComboboxItemContainer<WIA_IPS_CUR_INTENT> ColorMode_Color = new(wiaIntent_Color,  L_SCAN_PARAMS_MODE_COLOR_IMAGE);
	private static readonly ComboboxItemContainer<WIA_IPS_CUR_INTENT> ColorMode_GrayScale = new(wiaIntent_GrayScale,  L_SCAN_PARAMS_MODE_GRAYSCALE);
	private static readonly ComboboxItemContainer<WIA_IPS_CUR_INTENT> ColorMode_BWText = new(wiaIntent_BWText,  L_SCAN_PARAMS_MODE_BW_TEXT);
	...
	internal ScannerDevice? GetSelectedScanner()
		=> cboScannerSelect.eSelectedItemAs_ObjectContainerValue<ScannerDevice>();
	...
	internal SizeF? ScannerPaperSourceSizeCm => GetSelectedPaperSource().UseAutoFeed
		? GetSelectedScanner()?.Caps.Feeder_Caps?.SheetSizeIn.eInchesToCM()
		: GetSelectedScanner()?.Caps.FlatBed_Caps?.SizeIn.eInchesToCM();


	internal (WIA_IMAGE_FORMATS? format, int QualityPercent) GetSelectedFileFormat()
	{
		WIA_IMAGE_FORMATS? format = cboFileFormat.eSelectedItemAs_ObjectContainerValue<WIA_IMAGE_FORMATS>();
		int quality = 0;

		if (format.HasValue && format.Value.eHasCompressionQualityParameter())
		{
			quality = sldFile_Quality.Value;
		}
		...
		try
		{
...

			{
				...
				var selectedDPI = (cboDPI.SelectedItem == null)
					? minDPI
					: cboDPI.eSelectedItemAs_ObjectContainerValue<int>();

				int[] validDPIList = dpiList
					.Where(dpi => dpi >= minDPI && dpi <= maxDPI)
					.ToArray();

				cboDPI.eFillWithContainersOf<int>(validDPIList, selectedDPI);
			}
	...
	private void On_PaperSourceChanged()
	{
		...
		lblPageSize.Text = !pageSizeCm.HasValue
			? null
			:  L_SCAN_PARAMS_SCANNER_PAGE_SIZE.eFormat(pageSizeCm.Value.ToString_WxH(1));

		/*
				var DPI = Selected_DPI;
				var rcPagePixels = rcPageInches.eInches_To_Pixels(DPI);
	...
	void SetCropAreaCm(RectangleF rcfCrop)
	{
		...
		try
		{
			xudCrop_Left.Value = (decimal)rcfCrop.Left.eRound(2);
			xudCrop_Top.Value = (decimal)rcfCrop.Top.eRound(2);

			decimal cw = (decimal)(rcfCrop.Width.eRound(2));
			decimal ch = (decimal)(rcfCrop.Height.eRound(2));
	...
	private void On_FileFormatChanged()
	{
		...
		(WIA_IMAGE_FORMATS? format, int quality) = GetSelectedFileFormat();
		bool formatHasQuality = format.HasValue && format.Value.eHasCompressionQualityParameter();

		if (formatHasQuality)
		{
			sldFile_Quality.Value = quality;
	...
	private void On_FileQualityChanged()
	{
		...
		//sldFile_Quality.Visible = formatHasQuality;
		//lblFile_Quality.Visible = formatHasQuality;
		//lblFile_Quality.Text = $"{quality}%";
		float q = quality;
		q = q / 100;
		lblFile_Quality.Text =  L_SCAN_PARAMS_FILE_QUALITY.eFormat(q).eWrap(); ;

		//this.UpdateCurrentScanProfileSettingsFromUI();
		//lblFile_Quality.Text = WIA_Scanner.eExecExtensions_Strings.eFormat("Качество:|{0}%", (object)this._CurrentScanProfile.ImageQuality.ToString().Trim()).eWrapVB();
	}
